/**
 * Color Grading Lab Console — Standalone Module
 *
 * Coordinates states and WebGL/PixiJS render loops for the new Color Grading Lab.
 */
...
/** Merges presets parameters with manual overrides according to precedence rules. */
export function mergeGradingParams(
  ...
  if (manualAdjustments) {
    ...
    if ('hue' in manualAdjustments) merged.hueRotate = (manualAdjustments.hue! * Math.PI) / 180; // map degrees to radians
    ...
    // Advanced Grading — Replaced WHOLE when present, never partially deep-merged
...
export function ColorGradingLabView() {
  // ── Initialization ────────────────────────────────────────────────────────
  useEffect(() => {
    try {
  ...
  // ── Filter Selection & Manual Adjustments ──────────────────────────────────
  const [selectedFilterId, setSelectedFilterId] = useState<string>(IDENTITY_FILTER_ID);
  const [intensity, setIntensity] = useState(100);
  const [manualAdjustments, setManualAdjustments] = useState<ColorAdjustments>({});
  ...
  const [logs, setLogs] = useState<string[]>([
    "[INIT] Color grading GPU pipeline active.",
    "[OK] PixiRenderer WebGL engine active.",
  ...
  const pixiRendererRef = usePixiRenderer(
    ...
    useCallback((renderer) => {
      ...
      // Setup shader effect graph — ONLY color-adjustments node
   ...
   * = preset.gradingParams (base look) merged with manualAdjustments (user overrides).
   */
  const effectiveGradingParams = useMemo(() => {
    return mergeGradingParams(selectedFilter?.gradingParams, manualAdjustments);
  ...
  const syncAdjustmentsUniformsDirect = useCallback(
    (filter: typeof selectedFilter, inst: number, adjusts: ColorAdjustments) => {
      const renderer = pixiRendererRef.current;
      if (!renderer || !renderer.isReady) return;

      const f = inst / 100;

      // Initialize base parameters with identity defaults
      let base = {
      ...
      // 1. Resolve preset parameters scaled by intensity factor `f`
      const presetParams = filter?.gradingParams;
      if (filter && !presetParams && filter.cssFilter) {
      ...
      if (presetParams) {
        if (presetParams.exposure != null) base.exposure = presetParams.exposure * f;
        if (presetParams.brightness != null) base.brightness = presetParams.brightness * f;
        if (presetParams.contrast != null) base.contrast = presetParams.contrast * f;
        if (presetParams.saturation != null) base.saturation = presetParams.saturation * f;
        if (presetParams.temperature != null) base.temperature = presetParams.temperature * f;
        if (presetParams.tint != null) base.tint = presetParams.tint * f;
        if (presetParams.sepia != null) base.sepia = presetParams.sepia * f;
        if (presetParams.grayscale != null) base.grayscale = presetParams.grayscale * f;
        if (presetParams.hueRotate != null) base.hueRotate = presetParams.hueRotate * f;
        if (presetParams.vignette != null) base.vignette = presetParams.vignette * f;
        if (presetParams.invert != null) base.invert = presetParams.invert * f;
        if (presetParams.lift != null) base.lift = presetParams.lift * f;
        ...
        if (presetParams.grain) {
          base.grainIntensity = presetParams.grain.intensity * f;
          base.grainSize = presetParams.grain.size;
        }
        if (presetParams.channelMix) {
          base.channelMixR = presetParams.channelMix.r;
          base.channelMixG = presetParams.channelMix.g;
          base.channelMixB = presetParams.channelMix.b;
          base.useChannelMix = 1.0;
        }
        if (presetParams.duotone) {
          const [dr, dg, db] = hexToRgb(presetParams.duotone.darkColor);
          const [lr, lg, lb] = hexToRgb(presetParams.duotone.lightColor);
        ...
        if (presetParams.vibrance) {
          base.vibranceAmount = presetParams.vibrance.amount * f;
          if (presetParams.vibrance.protectedHue) {
            const [pr, pg, pb] = hexToRgb(presetParams.vibrance.protectedHue);
            base.vibranceProtectedHueR = pr;
            base.vibranceProtectedHueG = pg;
            base.vibranceProtectedHueB = pb;
          }
        }
        if (presetParams.crossProcess) {
          base.crossProcessAmount = presetParams.crossProcess.amount * f;
        }
      }

      // 2. Overwrite with manual overrides (absolute, not scaled by `f`)
      if ('exposure' in adjusts && adjusts.exposure !== undefined) {
        base.exposure = adjusts.exposure;
      }
      if ('brightness' in adjusts && adjusts.brightness !== undefined) {
        base.brightness = adjusts.brightness;
      }
      if ('contrast' in adjusts && adjusts.contrast !== undefined) {
        base.contrast = adjusts.contrast;
      }
      if ('saturation' in adjusts && adjusts.saturation !== undefined) {
        base.saturation = adjusts.saturation;
      }
      if ('temperature' in adjusts && adjusts.temperature !== undefined) {
        base.temperature = adjusts.temperature;
      }
      if ('tint' in adjusts && adjusts.tint !== undefined) {
        base.tint = adjusts.tint;
      }
      if ('sepia' in adjusts && adjusts.sepia !== undefined) {
        base.sepia = adjusts.sepia;
      }
      if ('grayscale' in adjusts && adjusts.grayscale !== undefined) {
        base.grayscale = adjusts.grayscale;
      }
      if ('hue' in adjusts && adjusts.hue !== undefined) {
        base.hueRotate = (adjusts.hue * Math.PI) / 180;
      }
      if ('vignette' in adjusts && adjusts.vignette !== undefined) {
        base.vignette = adjusts.vignette;
      }
      if ('invert' in adjusts && adjusts.invert !== undefined) {
        base.invert = adjusts.invert ? 1.0 : 0.0;
      }
      if ('lift' in adjusts && adjusts.lift !== undefined) {
        base.lift = adjusts.lift;
      }
      if ('vibrance' in adjusts && adjusts.vibrance !== undefined) {
        base.vibranceAmount = adjusts.vibrance.amount;
        if (adjusts.vibrance.protectedHue) {
          const [pr, pg, pb] = hexToRgb(adjusts.vibrance.protectedHue);
          base.vibranceProtectedHueR = pr;
          base.vibranceProtectedHueG = pg;
          base.vibranceProtectedHueB = pb;
        }
      }
      if ('grain' in adjusts && adjusts.grain !== undefined) {
        base.grainIntensity = adjusts.grain.intensity;
        base.grainSize = adjusts.grain.size;
      }
      if ('crossProcess' in adjusts && adjusts.crossProcess !== undefined) {
        base.crossProcessAmount = adjusts.crossProcess.amount;
      }

      renderer.updateParams("color-adjustments-node", base);
  ...
  const handleAdjustmentChange = (key: keyof ColorAdjustments, val: any) => {
    setManualAdjustments((prev) => {
      const next = { ...prev, [key]: val };
      addLog(`[EFFECT] Adjusted ${key} to direct manual value`);
      return next;
    });
  };

  const handleResetSlider = (key: keyof ColorAdjustments) => {
    setManualAdjustments((prev) => {
      const next = { ...prev };
      delete next[key];
      addLog(`[EFFECT] Reset override for key: ${key}`);
      return next;
    });
  };
  ...
  const handleResetAllAdjustments = () => {
    ...
    addLog("[OK] Reset all manual adjustments overrides.");
  };
  ...
  const handleResetAll = () => {
    ...
    setIntensity(100);
    addLog("[OK] Clear all manual adjustments and selected presets.");
  };
  ...
  const handleExportFrame = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `color-graded-snapshot-${selectedFilterId}-${Date.now()}.png`;
      a.click();
  ...
  const handleGenerateFilter = async () => {
    ...
    try {
      ...
      const randomAdjustments: ColorAdjustments = {
        exposure: parseFloat((-0.15 + Math.random() * 0.3).toFixed(2)),
        brightness: parseFloat((-0.10 + Math.random() * 0.2).toFixed(2)),
        contrast: parseFloat((Math.random() * 0.25).toFixed(2)),
        saturation: parseFloat((-0.30 + Math.random() * 0.5).toFixed(2)),
        temperature: parseFloat((-0.40 + Math.random() * 0.8).toFixed(2)),
        tint: parseFloat((-0.20 + Math.random() * 0.4).toFixed(2)),
        sepia: Math.random() > 0.6 ? parseFloat((Math.random() * 0.3).toFixed(2)) : 0,
        grayscale: 0,
        hue: Math.floor(-20 + Math.random() * 40),
        vignette: parseFloat((0.15 + Math.random() * 0.4).toFixed(2)),
        invert: false,
        lift: parseFloat((-0.1 + Math.random() * 0.2).toFixed(2)),
        vibrance: { amount: parseFloat((Math.random() * 0.4).toFixed(2)), protectedHue: "#E8B08C" },
        grain: { intensity: parseFloat((Math.random() * 0.15).toFixed(2)), size: 1.2 },
        crossProcess: { amount: parseFloat((Math.random() * 0.4).toFixed(2)) }
      };
      ...
      setAiStatus("success");
      setAiMessage(`AI synthesized look "${aiCategory.toUpperCase()}_GRADED" loaded.`);
  ...
  const handleOpenPublishModal = () => {
    ...
    addLog("[DEPLOY] Opening publish modal — review grading params payload.");
  ...
  const handleResetContext = () => {
    ...
    setLogs([
      ...
      "[OK] GPU color grading pipeline cleared.",
      "[INFO] Ready. Load media or select presets."
    ]);
  };

  const handleDumpLog = () => {
    console.group("%c[COLOR GRADING DIAG LOG DUMP]", "color:#7C6FFF;font-weight:bold");
  ...
  const histogramSVGData = useMemo(() => {
    ...
    return {
      r: getPathData(histogramData.r),
      g: getPathData(histogramData.g),
      b: getPathData(histogramData.b),
      l: getPathData(histogramData.l),
    };
  }, [histogramData]);
...
export default ColorGradingLabView;
