...
function hexToRgb(value: string): [number, number, number] {
  ...
  return [0, 2, 4].map((offset) => Number.parseInt(clean.slice(offset, offset + 2), 16) / 255) as [number, number, number];
}

function toNativeGrade(preset: (typeof PRESET_FILTERS)[number] | undefined, intensity: number): NativeGrade {
  ...
  if (params?.crossProcess) grade.crossProcessAmount = params.crossProcess.amount * weight;
...
export function NativeFilterLabView() {
  ...
  const [nativeState, setNativeState] = useState<"idle" | "ready" | "error">("idle");
  ...
  const preset = useMemo(() => PRESET_FILTERS.find((item) => item.id === presetId), [presetId]);
  ...
  const renderNative = useCallback(async () => {
    ...
    try {
      ...
      const request: NativeLabFrameRequest = {
        ...
        frameTime: { frameIndex: frame, ticks: Math.round(frame * 1_000_000 / 60), timescale: 1_000_000 },
        project: {
          schemaVersion: 1,
    ...
    <div className="min-h-screen bg-[#020617] text-white p-6">
      <div className="mx-auto max-w-6xl space-y-5">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-mono uppercase tracking-[0.2em] text-[#7c6fff]">Native Lab</p>
            <h1 className="text-2xl font-semibold">Filter Lab</h1>
            <p className="mt-1 text-sm text-slate-400">Raster upload at the browser boundary, color grading and composition in Rust/wgpu.</p>
          </div>
          <span className={`rounded border px-3 py-1 text-xs font-mono ${nativeState === "ready" ? "border-emerald-500/40 text-emerald-300" : nativeState === "error" ? "border-amber-500/40 text-amber-300" : "border-slate-700 text-slate-400"}`}>
            {nativeState === "ready" ? "NATIVE GPU READY" : nativeState === "error" ? "NATIVE RENDER ERROR" : "WAITING FOR LOCAL DAEMON"}
          </span>
        </header>
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_280px]">
          <section className="rounded-xl border border-slate-800 bg-slate-950 p-4">
            <div className="aspect-video overflow-hidden rounded-lg bg-black">
              <canvas ref={outputRef} width={WIDTH} height={HEIGHT} className="h-full w-full object-contain" />
            </div>
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <label className="cursor-pointer rounded bg-[#7c6fff] px-3 py-2 text-sm font-medium hover:bg-[#6859ff]">
                Load image/video
                <input type="file" accept="image/*,video/*" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) loadFile(file); }} />
              </label>
              {isVideo && <button type="button" onClick={() => setPlaying((value) => !value)} className="rounded border border-slate-700 px-3 py-2 text-sm">{playing ? "Pause" : "Play"}</button>}
              <span className="text-xs text-slate-500">Frame {frame}</span>
            </div>
            {error && <p className="mt-3 rounded border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200">{error}</p>}
          </section>

          <aside className="space-y-4 rounded-xl border border-slate-800 bg-slate-950 p-4">
            <label className="block text-xs font-mono uppercase tracking-wider text-slate-400">Native preset</label>
            <select value={presetId} onChange={(event) => setPresetId(event.target.value)} className="w-full rounded border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white">
              {PRESET_FILTERS.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
            </select>
            <label className="block text-xs font-mono uppercase tracking-wider text-slate-400">Intensity: {intensity}%</label>
            <input type="range" min="0" max="100" value={intensity} onChange={(event) => setIntensity(Number(event.target.value))} className="w-full" />
            <div className="rounded border border-slate-800 bg-slate-900/60 p-3 text-xs leading-relaxed text-slate-400">
              This workspace does not load a compatibility renderer. Unsupported native capabilities return an explicit daemon error.
            </div>
          </aside>
        </div>

        <video ref={videoRef} src={isVideo ? mediaUrl ?? undefined : undefined} className="hidden" muted playsInline onLoadedData={() => void renderNative()} onTimeUpdate={() => setFrame(Math.floor((videoRef.current?.currentTime ?? 0) * 60))} />
        <img ref={imageRef} src={!isVideo ? mediaUrl ?? undefined : undefined} alt="" className="hidden" onLoad={() => void renderNative()} />
        <canvas ref={sourceRef} width={WIDTH} height={HEIGHT} className="hidden" />
      </div>
    </div>
  );
}
