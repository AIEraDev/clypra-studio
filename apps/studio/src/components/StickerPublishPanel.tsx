...
const extractLottieThumbnail = (
  lottieJson: any,
  targetFrame?: number,
): Promise<{ dataUrl: string; totalFrames: number }> => {
      ...
      anim.addEventListener("DOMLoaded", () => {
        ...
        const frameToUse =
          targetFrame !== undefined
            ...
            : Math.min(
                Math.max(0, Math.floor(totalFrames * 0.1)),
                totalFrames - 1,
              );
...
export function StickerPublishPanel({
  ...
  const [previewGifDataUrl, setPreviewGifDataUrl] = useState<string | null>(
    null,
  );
  ...
  const handleGifSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(`Failed to process GIF file: ${err.message}`);
  ...
  const handleLottieSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.endsWith(".json") && file.type !== "application/json") {
      toast.error("Please select a JSON file for Lottie animations");
      return;
    }
    ...
    try {
      ...
      const { dataUrl, totalFrames: extractedTotal } =
        await extractLottieThumbnail(json);
      setTotalFrames(extractedTotal);

      const defaultFrame = Math.min(
        Math.max(0, Math.floor(extractedTotal * 0.1)),
        extractedTotal - 1,
      );
      ...
      const thumbFile = dataURLtoFile(
        dataUrl,
        `${file.name.replace(".json", "")}-thumb.png`,
      );
      setImageFile(thumbFile);
    } catch (err: any) {
      toast.error(`Failed to process Lottie file: ${err.message}`);
  ...
  const handleFrameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    debounceTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const thumbFile = dataURLtoFile(
          dataUrl,
          `${animatedFile?.name.replace(".json", "")}-thumb.png`,
        );
        setImageFile(thumbFile);
      } catch (err: any) {
        toast.error(`Failed to extract frame: ${err.message}`);
      } finally {
        setExtractingThumbnail(false);
      }
    }, 150);
  ...
  const handleGenerateMetadata = async () => {
    if (!imagePreview) {
      toast.error("Please select a Lottie JSON file first");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            errorData.error ||
            "Failed to generate sticker metadata",
        );
      }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      toast.success(result.message || "Sticker uploaded to R2 successfully!");
      setFormData({
        id: "",
        name: "",
        category: "emoji",
    ...
    >
      ...
      >
        <div className="flex items-start gap-3">
          <span
            ...
            } flex shrink-0 items-center justify-center rounded-lg border border-[#7C6FFF]/30 bg-[#7C6FFF]/10 text-[#B9B2FF]`}
          >
            <Upload size={isWorkspace ? 20 : 16} />
          </span>
          <div className="flex-1">
            <p className="text-[10px] font-bold uppercase tracking-wider text-[#B9B2FF]">
              Sticker Library
            </p>
            <h3 className={`${isWorkspace ? "text-xl" : "text-sm"} font-bold`}>
              Publish Sticker to API
            </h3>
            <p
              className={`${
            ...
            >
              Publish Lottie animated stickers to the R2 bucket immediately.
            </p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <section className="rounded-xl border border-[#2A2A38] bg-[#101018] p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="e.g., Fire Emoji"
                className="flex-1 px-3 py-1.5 bg-[#09090D] border border-[#2A2A38] rounded-lg text-white placeholder:text-gray-500 focus:outline-none focus:border-[#7C6FFF] text-xs"
              />
              <button
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-1.5 bg-[#09090D] border border-[#2A2A38] rounded-lg text-gray-400 focus:outline-none focus:border-[#7C6FFF] text-xs font-mono"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                handleCategoryChange(e.target.value as StickerCategory)
              }
              className="w-full px-3 py-1.5 bg-[#09090D] border border-[#2A2A38] rounded-lg text-white focus:outline-none focus:border-[#7C6FFF] text-xs"
            >
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., fire, emoji, hot"
              className="w-full px-3 py-1.5 bg-[#09090D] border border-[#2A2A38] rounded-lg text-white placeholder:text-gray-500 focus:outline-none focus:border-[#7C6FFF] text-xs"
            />
          </div>
          ...
          {isAdmin && (
            <div className="flex items-center gap-2.5 p-3 rounded-lg border border-[#2A2A38] bg-[#0E0E12] select-none">
              ...
              />
              <label
                htmlFor="sticker-publish-checkbox"
        ...
        <section className="rounded-xl border border-[#2A2A38] bg-[#101018] p-4 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-300 mb-1.5">
              Animation File <span className="text-red-400">*</span> (Lottie
              JSON)
            </label>
                ...
                <div className="border-2 border-dashed border-[#2A2A38] rounded-lg p-5 hover:border-[#7C6FFF] transition-colors bg-[#09090D]">
                  <div className="flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-xs text-gray-300">
                      {animatedFile
                        ? animatedFile.name
                        : "Click to select Lottie JSON file"}
                    </p>
                    <p className="text-[10px] text-gray-500 mt-1">
                      Accepts only JSON files representing Lottie vector
                      animations
                    </p>
                  </div>
                </div>
                ...
                <label className="cursor-pointer block">
                  <div className="border-2 border-dashed border-[#2A2A38] rounded-lg p-4 hover:border-[#7C6FFF] transition-colors bg-[#09090D]">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-xs text-gray-300">
                        {customPreviewFile
                          ? customPreviewFile.name
                          : "Click to upload a custom .gif preview"}
                      </p>
                      <p className="text-[10px] text-gray-500 mt-0.5">
                        Use this to provide a custom .gif preview instead of
                        generating one
                      </p>
                    </div>
                  </div>
                  ...
                  {totalFrames > 0 && (
                    <div className="p-3 bg-[#09090D] border border-[#2A2A38] rounded-lg space-y-2">
                      ...
                      <p className="text-[10px] text-gray-500">
                        Drag to select a different frame as the static
                        thumbnail.
                      </p>
                    </div>
                  )}
        ...
        <section className="rounded-xl border border-[#2A2A38] bg-[#101018] p-4 space-y-4">
          ...
          <div className="rounded-lg border border-[#2A2A38] bg-[#09090D] p-3 text-xs text-[#9A9AAA]">
            ...
            <p className="text-[11px] leading-relaxed">
              Ensure Lottie coordinates are centered. Large animation sequences
              must use compressed SVG path vectors to guarantee fast load times
              on devices.
            </p>
          </div>
          ...
          <button
            ...
            disabled={publishing || extractingThumbnail || !animatedFile}
            className="w-full py-2.5 bg-[#7C6FFF] hover:bg-[#6C5FEF] disabled:bg-[#2A2A38] disabled:text-gray-500 text-white font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 text-xs cursor-pointer"
          >
            {publishing ? (
              <>
          ...
          <p className="text-[10px] text-gray-500 text-center">
            Uploading directly packages and deploys assets to R2.
          </p>
        </section>
      </div>
    </div>
