...
import { X, UploadCloud, Loader2, AlertTriangle, CheckCircle, FileJson, Image as ImageIcon, Code2, ChevronDown, ChevronUp, Sliders, Layers, Check, Minus } from "lucide-react";
...
export function PublishFilterModal({ open, onClose, selectedFilter, gradingParams, thumbnailDataUrl, allPresets, onPublished }: PublishFilterModalProps) {
  ...
  const r2Payload = useMemo(
    ...
    [filterId, filterName, category, description, intensity, prunedParams, creatorName, thumbnailDataUrl, isAdmin],
  );
  ...
  function validate(): ValidationErrors {
    ...
    if (!hasNonZeroParams) errors.gradingParams = "No grading params set — adjust at least one slider in the Filter Lab before publishing. An empty filter does nothing in the editor.";
    return errors;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="flex h-[88vh] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-[#2A2A38] bg-[#0E0E12] shadow-2xl">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h3 className="text-sm font-bold text-white">{isBatchMode ? "Upload All Presets to R2" : "Publish GPU Filter to API"}</h3>
        ...
        <div className="flex border-b border-[#2A2A38] bg-[#15151C] shrink-0">
          ...
          {isBatchMode && (
            <button onClick={() => setActiveTab("batch")} className={`flex-1 py-3 text-center text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors ${activeTab === "batch" ? "text-indigo-300 bg-[#121219] border-b-2 border-indigo-500" : "text-[#888899] hover:text-white"}`}>
          ...
          {activeTab === "batch" && allPresets ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => setSelectedPresetIds(new Set(allPresets.map((p) => p.id)))} className="text-[10px] font-semibold text-indigo-300 hover:text-white transition-colors">
                    Select all
                  </button>
                  <span className="text-[#2A2A38]">|</span>
                  <button type="button" onClick={() => setSelectedPresetIds(new Set())} className="text-[10px] font-semibold text-[#888899] hover:text-white transition-colors">
                    Deselect all
                  </button>
                </div>
              </div>
                ...
                {allPresets.map((preset) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors disabled:cursor-not-allowed ${isSelected ? "bg-indigo-500/10" : "bg-[#0B0B10] hover:bg-[#141420]"}`}
                    >
                      {/* Checkbox */}
                      <div className={`w-4 h-4 rounded border shrink-0 flex items-center justify-center ${isSelected ? "border-indigo-500 bg-indigo-500" : "border-[#2A2A38]"}`}>{isSelected && <Check size={10} className="text-white" />}</div>
                      ...
                      {/* Category badge */}
                      <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded bg-[#1A1A24] text-[#888899] shrink-0">{preset.category}</span>

                      {/* Param count */}
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 ${paramCount > 0 ? "bg-indigo-500/15 text-indigo-300" : "bg-amber-900/30 text-amber-400"}`}>
                        {paramCount} param{paramCount !== 1 ? "s" : ""}
                      </span>

                      {/* Upload result badge */}
                      {result && <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 ${result.ok ? "bg-teal-500/15 text-teal-300" : "bg-red-900/30 text-red-400"}`}>{result.ok ? "✓" : "✗"}</span>}
                      {/* In-progress indicator */}
                      {isUploading && batchProgress.current === preset.name && !result && <Loader2 size={12} className="animate-spin text-indigo-300 shrink-0" />}
                    </button>
                  );
                })}
              </div>
              ...
              {isUploading && batchProgress.total > 0 && (
                <div className="space-y-1.5">
                  <div className="flex justify-between text-[10px] text-[#888899]">
                    <span>
                      Uploading <span className="text-white font-semibold">{batchProgress.current}</span>
                    </span>
                    <span>
                  ...
                  <div className="h-1.5 rounded-full bg-[#1A1A24] overflow-hidden">
                    <div className="h-full bg-indigo-500 transition-all duration-300 rounded-full" style={{ width: `${(batchProgress.done / batchProgress.total) * 100}%` }} />
                  </div>
                </div>
              )}
              ...
              {!isUploading && batchResults.length > 0 && (
                <div className={`rounded-xl border p-3 ${uploadStatus === "success" ? "border-teal-900/50 bg-teal-950/20" : "border-amber-900/50 bg-amber-950/20"}`}>
                  <p className={`text-xs font-bold mb-1 ${uploadStatus === "success" ? "text-teal-300" : "text-amber-300"}`}>{uploadMessage}</p>
                  <div className="space-y-1 max-h-28 overflow-y-auto">
                    {batchResults.map((r) => (
                      <div key={r.id} className="flex items-center gap-2 text-[10px]">
                        {r.ok ? <Check size={10} className="text-teal-400 shrink-0" /> : <Minus size={10} className="text-red-400 shrink-0" />}
            ...
            <>
              ...
              <div>
                ...
                <p className="mt-1.5 text-[10px] text-[#555566]">
                  Used in R2 path: filters/{category}/{filterId || "…"}.json
                </p>
              </div>
              ...
              <div>
                ...
                <select value={category} onChange={(e) => setCategory(e.target.value as FilterCategoryId)} className="w-full rounded-lg border border-[#2A2A38] bg-[#09090D] px-3 py-2 text-xs text-white outline-none focus:border-teal-500">
              ...
              <div>
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="A warm, desaturated cinematic grade with subtle vignette…" rows={2} className="w-full rounded-lg border border-[#2A2A38] bg-[#09090D] px-3 py-2 text-xs text-white outline-none placeholder:text-[#555566] focus:border-teal-500 resize-none" />
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(["Light", "Medium", "Bold"] as const).map((lvl) => (
                    <button key={lvl} type="button" onClick={() => setIntensity(lvl)} className={`flex-1 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${intensity === lvl ? "bg-teal-500/20 border-teal-500 text-teal-300" : "border-[#2A2A38] text-[#888899] hover:border-[#3A3A48] hover:text-white"}`}>
                      {lvl}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <input type="text" value={creatorName} onChange={(e) => setCreatorName(e.target.value)} placeholder="Your name" className="w-full rounded-lg border border-[#2A2A38] bg-[#09090D] px-3 py-2 text-xs text-white outline-none placeholder:text-[#555566] focus:border-teal-500" />
              </div>
            </>
          ) : (
            ...
            <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                    {Object.entries(prunedParams).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between gap-2 rounded bg-[#09090D] border border-[#1A1A24] px-2.5 py-1.5">
                        ...
                        <span className="font-mono text-[10px] font-bold text-teal-300">{typeof value === "number" ? value.toFixed(3) : JSON.stringify(value)}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-amber-400/80 leading-relaxed">
                          All sliders are at zero. This JSON will be stored in R2 with an empty <code className="text-amber-300">gradingParams</code>, and the editor's <code className="text-amber-300">ColorAdjustmentsEffect</code> will apply all-zero GLSL uniforms — the filter will look identical to no filter at all.
                        </p>
                        <p className="mt-2 text-[10px] text-amber-300 font-semibold">→ Close this modal, adjust the grading sliders in Filter Lab, then re-open.</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="rounded-xl border border-[#2A2A38] bg-[#0B0B10] p-4">
                ...
                <p className="text-[10px] text-[#555566] mb-3">
                  Exact body POSTed to <code className="text-teal-400">/filters/upload</code>
                </p>
                {jsonExpanded ? (
                  <pre className="overflow-x-auto rounded-lg border border-[#1A1A24] bg-[#09090D] p-3 font-mono text-[10px] text-teal-200 leading-relaxed whitespace-pre-wrap">{JSON.stringify(r2Payload, null, 2)}</pre>
                ) : (
        ...
        <div className="border-t border-[#2A2A38] bg-[#15151C] p-4 shrink-0 space-y-3">
          {uploadMessage && (
            <div className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-[10px] ${isError ? "border-red-900/40 bg-red-950/30 text-red-300" : isSuccess ? "border-teal-900/40 bg-teal-950/30 text-teal-300" : "border-[#2A2A38] bg-[#0B0B10] text-[#9A9AAA]"}`}>
              {isError ? <AlertTriangle size={14} className="shrink-0" /> : isSuccess ? <CheckCircle size={14} className="shrink-0" /> : <Loader2 size={14} className="shrink-0 animate-spin" />}
          ...
          <div className="flex justify-end gap-2">
            ...
            {isBatchMode ? (
              <button type="button" onClick={handleBatchUpload} disabled={isUploading || selectedPresetIds.size === 0} className="rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40 flex items-center gap-2">
                {isUploading ? (
                  <>
              ...
              <button type="button" onClick={handleUpload} disabled={isUploading || isSuccess || !hasNonZeroParams} title={!hasNonZeroParams ? "Adjust at least one slider in Filter Lab before publishing" : undefined} className="rounded-lg bg-teal-500 px-4 py-2 text-xs font-bold text-black hover:bg-teal-400 disabled:cursor-not-allowed disabled:opacity-40 flex items-center gap-2">
                {isUploading ? (
                  <>
                    <Loader2 size={14} className="animate-spin" />
                    Uploading…
