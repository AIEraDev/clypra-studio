...
export function AdminTransitionsSettings() {
  ...
  const loadTransitions = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to load transitions.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTogglePublished = async (
    ...
    try {
      const response = await fetch(
        `${getApiBaseUrl()}/transitions/${category}/${id}/publish`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          throw new Error(
            "Unauthorized: Only administrators can modify transitions.",
          );
        }
        if (response.status === 403) {
          throw new Error(
      ...
      setTransitions((prev) =>
        prev.map((t) =>
          t.id === id ? { ...t, published: !currentStatus } : t,
        ),
      );
      ...
      toast.error(error.message || "Failed to update transition status.");
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const transitionsByCategory = transitions.reduce((acc, transition) => {
    ...
    <div className="mx-auto max-w-6xl p-6 text-white">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-(--studio-muted)">
                Manage published status for all transitions. Only published
                transitions are visible in the Clypra app.
              </p>
            </div>
            <button
              type="button"
              ...
              disabled={loading}
              className="flex items-center gap-2 rounded-lg border border-(--studio-border) bg-(--studio-raised) px-4 py-2 text-xs font-medium hover:bg-(--studio-hover) disabled:opacity-50"
            >
      ...
      {loading && transitions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <Loader2
            size={32}
            className="animate-spin text-(--studio-accent) mb-4"
          />
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center rounded-xl border border-(--studio-border) bg-(--studio-panel)">
          ...
          <div className="text-[10px] text-(--studio-muted) mt-1">
            Upload transitions from the Transitions workspace
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          {categories.map((category) => {
            ...
            const publishedCount = categoryTransitions.filter(
              (t) => t.published,
            ).length;
            ...
            return (
              <div
                key={category}
                className="rounded-xl border border-(--studio-border) bg-(--studio-panel) overflow-hidden"
              >
                {/* Category header */}
                <div className="border-b border-(--studio-border) bg-(--studio-raised) px-6 py-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="rounded-full bg-green-500/10 px-3 py-1 text-[10px] font-medium text-green-400">
                        {publishedCount} published
                      </div>
                      <div className="rounded-full bg-(--studio-raised) border border-(--studio-border) px-3 py-1 text-[10px] font-medium text-(--studio-muted)">
                ...
                <div className="divide-y divide-(--studio-border)">
                  {categoryTransitions.map((transition) => (
                    <div
                      key={transition.id}
                      className="flex items-center justify-between px-6 py-4 hover:bg-(--studio-raised) transition-colors"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-[10px] text-(--studio-muted) font-mono bg-(--studio-raised) px-2 py-0.5 rounded">
                            {transition.id}
                          </div>
                        </div>
                        ...
                        {transition.tags && transition.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {transition.tags.slice(0, 5).map((tag) => (
                              <span
                                key={tag}
                                className="text-[10px] bg-(--studio-raised) border border-(--studio-border) px-2 py-0.5 rounded text-(--studio-muted)"
                              >
                                {tag}
                              </span>
                            ))}
                      ...
                      <button
                        ...
                        disabled={updatingId === transition.id}
                        className={`ml-4 flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                          transition.published
                            ? "bg-green-500/10 text-green-400 border border-green-500/30 hover:bg-green-500/15"
                            : "bg-(--studio-raised) text-(--studio-muted) border border-(--studio-border) hover:bg-(--studio-hover)"
                        }`}
                      >
                        {updatingId === transition.id ? (
