      ...
      <div className="flex-1 overflow-y-auto p-1.5 space-y-3 flex flex-col min-h-0">
          ...
          <>
            <div>
              ...
              <div className="property-grid bg-surface-container border border-outline-variant rounded select-none">
                      ...
                      <div className="text-on-surface select-none">
                        ...
                        {param.type === "color" && (
                          <div className="flex items-center gap-2">
                            <ClypraColorPicker
                              value={val || "#ffffff"}
                              onChange={(newColor) => onParamChange(param.key, newColor)}
                ...
                {(!activeTransition || activeTransition.params.length === 0) && <div className="col-span-2 text-center text-on-surface-variant p-2 text-[10px]">No parameters available for this transition</div>}
              </div>
            </div>
        ...
        {activeTab === "nodes" && (
          <div className="flex-1 p-2 bg-black border border-outline-variant font-mono-data text-[9.5px] leading-relaxed text-primary/80 overflow-y-auto">
            ...
            {activeTransition?.params.map((param) => (
              <p key={param.key} className="pl-4 text-on-surface-variant text-[9px]">
                {param.key}: {String(parameters[param.key] ?? param.value)}
              </p>
            ))}
          </div>
        )}
