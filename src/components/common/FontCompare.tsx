...
import { TextEffectConfig } from "@clypra-studio/engine";
import { TextEffectRenderer } from "@clypra-studio/engine";
import { X, ArrowLeft, ArrowRight } from "lucide-react";

export interface FontCompareProps {
...
const MiniFontCanvas: React.FC<{
  ...
  return <canvas ref={canvasRef} id={id} className="w-full h-[110px] rounded bg-[#0E0E12] border border-[#2A2A38] shadow-inner cursor-pointer" />;
};

export const FontCompare: React.FC<FontCompareProps> = ({ config, onSelectFont, onClose }) => {
  ...
  return (
    <div id="font-compare-overlay" className="absolute inset-0 bg-[#0E0E12]/95 border border-[#2A2A38] z-40 rounded-lg p-6 flex flex-col backdrop-blur-md overflow-y-auto">
      {/* Header */}
      ...
      {/* Grid */}
      <div id="font-compare-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 flex-1 content-start select-none">
      ...
      <div className="flex items-center justify-between border-t border-[#2A2A38] pt-4 mt-6">
        ...
        <div className="flex items-center gap-2">
          <button id="font-compare-prev" disabled={page === 0} onClick={handlePrev} className={`p-1.5 px-3 rounded text-xs flex items-center gap-1 border border-[#2A2A38] font-sans transition-all cursor-pointer ${page === 0 ? "text-gray-600 border-gray-800 bg-transparent cursor-not-allowed" : "text-white bg-[#1E1E26] hover:bg-[#2A2A38]"}`}>
          ...
          <button id="font-compare-next" disabled={page === totalPages - 1} onClick={handleNext} className={`p-1.5 px-3 rounded text-xs flex items-center gap-1 border border-[#2A2A38] font-sans transition-all cursor-pointer ${page === totalPages - 1 ? "text-gray-600 border-gray-800 bg-transparent cursor-not-allowed" : "text-white bg-[#1E1E26] hover:bg-[#2A2A38]"}`}>
