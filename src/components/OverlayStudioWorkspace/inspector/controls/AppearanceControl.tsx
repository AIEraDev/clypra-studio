...
function ShadowEditor({
  ...
  const set = <K extends keyof ShadowValue>(k: K, v: ShadowValue[K]) =>
    onChange({ ...value, [k]: v });
  return (
    <div className="flex flex-col gap-2 pl-2 border-l border-white/6 mt-1">
        ...
        {(["x", "y", "blur"] as const).map((k) => (
          <div key={k}>
            ...
            <input
              ...
              className="w-full bg-[#1C1C22] border border-white/6 rounded-md px-2 py-1 font-mono text-[11px] text-white focus:border-violet-500 focus:outline-none"
            />
          </div>
        ))}
        <div>
...
export function AppearanceControl({ value, onChange }: AppearanceControlProps) {
  ...
  const set = <K extends keyof AppearanceValue>(k: K, v: AppearanceValue[K]) =>
    onChange({ ...value, [k]: v });
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1">
            ...
            <ColorControl
              value={value.strokeColor || "#2E2E3E"}
          ...
          <div className="w-20 shrink-0">
            ...
            <input
              ...
              className="w-full h-7.5 bg-[#1C1C22] border border-white/6 rounded-md px-2 font-mono text-[11px] text-white focus:border-violet-500 focus:outline-none text-center"
            />
          </div>
        </div>
      </div>
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2 py-0.5 rounded-md border text-[10px] font-medium transition-colors cursor-pointer ${
              showShadow
                ...
                : "border-white/6 text-gray-500 hover:text-gray-300"
            }`}
          >
