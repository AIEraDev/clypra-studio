...
export interface PublishTransitionModalProps {
  open: boolean;
  onClose: () => void;
  transitionDef: {
    id: string;
...
export function PublishTransitionModal({
  ...
  useEffect(() => {
    if (!message) return;
    if (isPublished) toast.success(message, { id: "transition-publish" });
    else if (isFailed) toast.error(message, { id: "transition-publish" });
  ...
  useEffect(() => {
    if (open && transitionDef) {
      ...
      if (
        TRANSITION_CATEGORIES.includes(
          transitionDef.category as TransitionCategory,
        )
      ) {
  ...
  const r2Payload = useMemo(
    () => ({
      ...
      previewDataUrl: previewDataUrl
        ? "<base64 WebM — omitted for preview>"
        : null,
    }),
    [
      transitionId,
  ...
  const r2Paths = useMemo(
    () => ({
      ...
      thumbnail: thumbnailDataUrl
        ? `transitions/${category}/${transitionId || "<id>"}.png`
        : null,
      preview: previewDataUrl
        ? `transitions/${category}/${transitionId || "<id>"}.webm`
        : null,
      liveUrl: `https://clypra-worker-api.abdulkabirmusa.com/transitions/${category}/${
        transitionId || "<id>"
      }`,
    }),
  ...
  const handlePublish = async () => {
    ...
    if (!/^[a-z0-9-]+$/.test(transitionId)) {
      errors.id =
        "Transition ID must be kebab-case (lowercase, numbers, hyphens only)";
    }
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h3 className="text-sm font-bold text-white">
                  Publish GPU Transition to API
                </h3>
                <p className="mt-1 text-[11px] leading-relaxed text-[#9A9AAA]">
                  Review metadata and upload directly to Cloudflare R2
                </p>
              </div>
            </div>
            <button
            ...
            <>
              ...
              <div>
                ...
                <p className="mt-1.5 text-[10px] text-clypra-muted">
                  Unique kebab-case identifier (e.g., shatter-burst)
                </p>
              </div>
              ...
              <div>
                ...
                <select
                  value={category}
                  onChange={(e) =>
                    setCategory(e.target.value as TransitionCategory)
                  }
                ...
                <p className="mt-1.5 text-[10px] text-clypra-muted">
                  Primary visual category for index
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="mt-1.5 text-[10px] text-clypra-muted">
                  Brief description of the visual behavior
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDefaultDuration(parseFloat(e.target.value) || 1.0)
                  }
                  className="w-full rounded-lg border border-[#2A2A38] bg-[#09090D] px-3 py-2 text-xs text-white outline-none focus:border-teal-500"
                />
                <p className="mt-1.5 text-[10px] text-clypra-muted">
                  Initial duration when added to timeline
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mt-1.5 text-[10px] text-clypra-muted">
                  The speed curve of the transition progress
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 p-3 rounded-lg border border-[#2A2A38] bg-[#0E0E12] select-none">
                ...
                />
                <label
                  htmlFor="transition-premium-checkbox"
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-[#2A2A38] bg-[#0B0B10] p-4">
                <h4 className="text-xs font-bold text-white mb-3 flex items-center gap-2">
                  <FileJson size={14} className="text-teal-300" /> Transition
                  Info
                </h4>
              ...
              <div className="rounded-xl border border-[#2A2A38] bg-[#0B0B10] p-4">
                ...
                <div className="space-y-2">
                  {(
                    ...
                    >
                      <span className="text-[9px] font-bold uppercase text-[#888899] shrink-0 w-16">
                        {label}
                      </span>
                      <code
              ...
              {previewDataUrl && (
                <div className="rounded-xl border border-[#2A2A38] bg-[#0B0B10] p-4">
                  <h4 className="text-xs font-bold text-white mb-3 flex items-center gap-2">
                    <Video size={14} className="text-purple-300" /> Video
                    Preview (WebM → R2)
                  </h4>
              ...
              <div className="rounded-xl border border-[#2A2A38] bg-[#0B0B10] p-4">
                ...
                >
                  <span className="flex items-center gap-2">
                    <Code2 size={14} className="text-teal-300" /> JSON Payload
                    (POST /transitions/upload)
                  </span>
                  {jsonExpanded ? (
                    <ChevronUp size={13} />
                  ) : (
                ...
                <p className="text-[10px] text-[#555566] mb-3">
                  Exact body that will be sent to the API
                </p>
                {jsonExpanded && (
