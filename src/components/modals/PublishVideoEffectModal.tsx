...
export interface PublishVideoEffectModalProps {
  open: boolean;
  onClose: () => void;
  effectDef: {
    id: string;
...
export function PublishVideoEffectModal({
  ...
  const { uploadVideoEffect, status, message, reset } =
    useVideoEffectR2Upload();
  ...
  useEffect(() => {
    if (!message) return;
    if (isPublished) toast.success(message, { id: "video-effect-publish" });
    else if (isFailed) toast.error(message, { id: "video-effect-publish" });
  ...
  useEffect(() => {
    if (open && effectDef) {
      ...
      const catLower = (
        effectDef.category || ""
      ).toLowerCase() as VideoEffectCategory;
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 mt-0.5">
                Generate static asset URLs and make this effect discoverable in
                Clypra NLE.
              </p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={category}
                    onChange={(e) =>
                      setCategory(e.target.value as VideoEffectCategory)
                    }
                    disabled={isUploading || isPublished}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-gray-400">
                        Requires Clypra Pro subscription to render
                      </p>
                    </div>
                  </label>
                </div>
            ...
            <div className="text-[10px] text-gray-500">
              {isAdmin ? (
                <span className="text-amber-400 font-semibold">
                  ★ Admin Mode (Auto-Approves Publish)
                </span>
              ) : (
