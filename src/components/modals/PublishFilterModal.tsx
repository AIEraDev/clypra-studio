...
export interface PublishFilterModalProps {
  open: boolean;
  onClose: () => void;
...
export function PublishFilterModal({
  ...
  const [intensity, setIntensity] = useState<"Light" | "Medium" | "Bold">(
    "Medium",
  );
  ...
  const [batchResults, setBatchResults] = useState<
    Array<{ id: string; name: string; ok: boolean; msg: string }>
  >([]);

  // ── Tabs ───────────────────────────────────────────────────────────────────
  const [activeTab, setActiveTab] = useState<"metadata" | "preview" | "batch">(
    "metadata",
  );
  ...
  useEffect(() => {
    if (open) {
      ...
      setFilterName(
        selectedFilter?.name ? `${selectedFilter.name} Grade` : "Custom Grade",
      );
      const cat = (selectedFilter?.category as FilterCategoryId) ?? "cinematic";
      setCategory(
        FILTER_CATEGORY_OPTIONS.some((o) => o.id === cat) ? cat : "cinematic",
      );
      setDescription(
        selectedFilter?.description ??
          "A GPU color grading look from Filter Lab.",
      );
      setIntensity("Medium");
      setCreatorName("");
      setValidationErrors({});
  ...
  const prunedParams = useMemo(
    () => pruneGradingParams(gradingParams),
    [gradingParams],
  );
  ...
  const r2Payload = useMemo(
    () => ({
      ...
      thumbnailDataUrl: thumbnailDataUrl
        ? "<base64 PNG — omitted for preview>"
        : null,
    }),
    [
      filterId,
  ...
  const r2Paths = useMemo(
    () => ({
      ...
      thumbnail: thumbnailDataUrl
        ? `filters/${category}/${filterId || "<id>"}.png`
        : null,
      liveUrl: `https://clypra-worker-api.abdulkabirmusa.com/filters/${category}/${
        filterId || "<id>"
      }`,
    }),
  ...
  function validate(): ValidationErrors {
    ...
    else if (!/^[a-z0-9-]+$/.test(filterId))
      errors.id = "Must be kebab-case (lowercase, numbers, hyphens only)";
    if (!filterName.trim()) errors.name = "Filter name is required";
    if (!hasNonZeroParams)
      errors.gradingParams =
        "No grading params set — adjust at least one slider in the Filter Lab before publishing. An empty filter does nothing in the editor.";
    return errors;
  }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.success(result.message || "Filter uploaded successfully!");
      onPublished?.(
        filterId,
        result.message || "Filter uploaded successfully!",
      );
    } catch (err) {
  ...
  const handleBatchUpload = useCallback(async () => {
    ...
    for (let i = 0; i < toUpload.length; i++) {
      ...
      const presetParams = pruneGradingParams(
        (preset.gradingParams ?? {}) as GradingParams,
      );
      const presetCategory =
        (preset.category as FilterCategoryId) ?? "cinematic";
    ...
    const summary = `${results.length - failed}/${
      results.length
    } presets uploaded successfully.`;
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h3 className="text-sm font-bold text-white">
                  {isBatchMode
                    ? "Upload All Presets to R2"
                    : "Publish GPU Filter to API"}
                </h3>
                <p className="mt-1 text-[11px] leading-relaxed text-[#9A9AAA]">
                  {isBatchMode ? (
                    <>
                      ...
                      — each published with its own{" "}
                      <code className="text-indigo-300">gradingParams</code>
                    </>
                  ) : (
                    <>
                      Stores{" "}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 min-h-0">
          {activeTab === "batch" && allPresets ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-white">
                  {selectedPresetIds.size} / {allPresets.length} presets
                  selected
                </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-[#555566] truncate">
                          {preset.id}
                        </p>
                      </div>
                      ...
                      {isUploading &&
                        batchProgress.current === preset.name &&
                        !result && (
                          <Loader2
                            size={12}
                            className="animate-spin text-indigo-300 shrink-0"
                          />
                        )}
                    </button>
                  );
                  ...
                  <div className="space-y-1 max-h-28 overflow-y-auto">
                    {batchResults.map((r) => (
                      ...
                      >
                        ...
                        <span
                          className={r.ok ? "text-[#888899]" : "text-red-400"}
                        >
                          {r.name}
                        </span>
                        {!r.ok && (
                          <span className="text-[#555566] truncate">
                            {r.msg}
                          </span>
                        )}
                      </div>
            ...
            <>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (validationErrors.id)
                      setValidationErrors((p) => ({ ...p, id: undefined }));
                  }}
              ...
              <div>
                ...
                <select
                  value={category}
                  onChange={(e) =>
                    setCategory(e.target.value as FilterCategoryId)
                  }
              ...
              <div>
                ...
                <p className="mt-1.5 text-[10px] text-[#555566]">
                  Default slider value: Light=60, Medium=75, Bold=85
                </p>
              </div>
            ...
            <div className="space-y-4">
                ...
                <h4 className="text-xs font-bold text-white mb-1 flex items-center gap-2">
                  ...
                  <span
                    className={`ml-auto text-[9px] font-bold px-1.5 py-0.5 rounded ${
                      hasNonZeroParams
                        ? "bg-teal-500/15 text-teal-300"
                        : "bg-[#2A2A38] text-[#888899]"
                    }`}
                  >
                    {Object.keys(prunedParams).length} active uniform
                    {Object.keys(prunedParams).length !== 1 ? "s" : ""}
                  </span>
                </h4>
                <p className="text-[10px] text-[#555566] mb-3">
                  These GLSL uniforms are what's stored in R2 and fed into{" "}
                  <code className="text-teal-400">ColorAdjustmentsEffect</code>{" "}
                  on playback.
                </p>
                {hasNonZeroParams ? (
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                    {Object.entries(prunedParams).map(([key, value]) => (
                      <div
                        key={key}
                        className="flex items-center justify-between gap-2 rounded bg-[#09090D] border border-[#1A1A24] px-2.5 py-1.5"
                      >
                        <span className="font-mono text-[10px] text-[#888899]">
                          {key}
                        </span>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold text-amber-300 mb-1.5">
                          No grading parameters — filter will be invisible
                        </p>
                        <p className="text-[10px] text-amber-400/80 leading-relaxed">
                          All sliders are at zero. This JSON will be stored in
                          R2 with an empty{" "}
                          <code className="text-amber-300">gradingParams</code>,
                          and the editor's{" "}
                          <code className="text-amber-300">
                            ColorAdjustmentsEffect
                          </code>{" "}
                          will apply all-zero GLSL uniforms — the filter will
                          look identical to no filter at all.
                        </p>
                        <p className="mt-2 text-[10px] text-amber-300 font-semibold">
                          → Close this modal, adjust the grading sliders in
                          Filter Lab, then re-open.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {thumbnailDataUrl ? (
                  <div className="space-y-2">
                    <div className="relative rounded overflow-hidden border border-[#2A2A38] bg-[#09090D]">
                      <img
                        ...
                        alt="Filter thumbnail preview"
                        className="w-full h-auto"
                      />
                    </div>
                    <div className="flex items-center gap-2 rounded-lg bg-[#09090D] border border-[#2A2A38] px-3 py-2">
                      <span className="text-[9px] font-bold uppercase text-[#888899] shrink-0">
                        R2 path
                      </span>
                      <code className="flex-1 truncate font-mono text-[10px] text-amber-300">
                        {r2Paths.thumbnail}
                      </code>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-center h-20 rounded-lg border border-dashed border-[#2A2A38] bg-[#09090D]">
                    <p className="text-[10px] text-[#555566]">
                      No canvas frame captured — play/pause the video first
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="rounded-xl border border-[#2A2A38] bg-[#0B0B10] p-4">
                <h4 className="text-xs font-bold text-white mb-3 flex items-center gap-2">
                  <FileJson size={14} className="text-purple-300" /> R2 / API
                  Destinations
                </h4>
                <div className="space-y-2">
                  {[
                    {
                      label: "Endpoint",
                      value: r2Paths.apiEndpoint,
                      color: "text-orange-300",
                    },
                    {
                      label: "Index",
                      value: r2Paths.indexFile,
                      color: "text-teal-300",
                    },
                    {
                      label: "Live URL",
                      value: r2Paths.liveUrl,
                      color: "text-blue-300",
                    },
                  ].map(({ label, value, color }) => (
                    <div
                      key={label}
                      className="flex items-start gap-2 rounded bg-[#09090D] border border-[#1A1A24] px-3 py-2"
                    >
                      <span className="text-[9px] font-bold uppercase text-[#888899] shrink-0 w-14">
                        {label}
                      </span>
                      <code
                        className={`flex-1 break-all font-mono text-[10px] ${color}`}
                      >
                        {value}
                      </code>
                    </div>
              ...
              <div className="rounded-xl border border-[#2A2A38] bg-[#0B0B10] p-4">
                ...
                >
                  <span className="flex items-center gap-2">
                    <Code2 size={14} className="text-teal-300" /> Full JSON
                    Payload
                  </span>
                  {jsonExpanded ? (
                    <ChevronUp size={13} />
                ...
                <p className="text-[10px] text-[#555566] mb-3">
                  Exact body POSTed to{" "}
                  <code className="text-teal-400">/filters/upload</code>
                </p>
                {jsonExpanded ? (
            ...
            {isBatchMode ? (
              ...
              >
                {isUploading ? (
                  ...
                  <>
                    <UploadCloud size={14} />
                    Upload {selectedPresetIds.size} Preset
                    {selectedPresetIds.size !== 1 ? "s" : ""} to R2
                  </>
                )}
              </button>
            ) : (
              <button
                ...
                title={
                  !hasNonZeroParams
                    ? "Adjust at least one slider in Filter Lab before publishing"
                    : undefined
                }
