...
const ADDABLE_TYPES: EffectLayerType[] = ["glow", "shadow", "filter"];
...
export function LayerPanel({ scene, onSceneChange, uiMode }: LayerPanelProps) {
  if (uiMode === "basic") {
    return (
      <div className="px-4 py-3 border-b border-[#2A2A38] bg-[#12121A]">
        <p className="text-[10px] font-mono uppercase text-[#666677] tracking-wider mb-1">
          Effect layers
        </p>
  ...
  const toggleLayer = (id: string) => {
    updateLayers(
      scene.effectLayers.map((l) => (l.id === id ? { ...l, enabled: !l.enabled } : l))
    );
  };

  const setOpacity = (id: string, opacity: number) => {
    updateLayers(
      scene.effectLayers.map((l) => (l.id === id ? { ...l, opacity: opacity / 100 } : l))
    );
  };

  const removeLayer = (id: string) => {
    updateLayers(scene.effectLayers.filter((l) => l.id !== id));
  };
    ...
    <div className="border-b border-[#2A2A38] bg-[#12121A]">
        ...
        <div className="flex gap-1">
          {ADDABLE_TYPES.map((t) => (
            <button
              ...
              type="button"
              onClick={() => addLayer(t)}
              className="p-1 rounded hover:bg-[#2A2A38] text-gray-400 hover:text-white cursor-pointer"
              title={`Add ${t}`}
            >
              <Plus size={12} />
            </button>
      ...
      <ul className="max-h-48 overflow-y-auto custom-scrollbar">
        {scene.effectLayers.map((layer, index) => (
          <li
            key={layer.id}
            className="flex items-center gap-2 px-3 py-1.5 border-b border-[#1A1A26]/50 hover:bg-[#1A1A26]/40 text-xs"
          >
            <button
              type="button"
            ...
            <input
              ...
              onChange={(e) => setOpacity(layer.id, parseInt(e.target.value, 10))}
              className="w-14 h-1 accent-[#7C6FFF]"
              title="Opacity"
            />
            <button
              type="button"
            ...
            {ADDABLE_TYPES.includes(layer.type) && (
              <button
                ...
                className="p-0.5 text-gray-600 hover:text-red-400 cursor-pointer"
              >
                <Trash2 size={12} />
              </button>
            )}
