      ...
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-[#2A2A38] bg-[#121219] shadow-2xl">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] leading-relaxed text-[#9A9AAA]">Used for AI naming, prompt generation, image style scanning, and research. Stored locally in this browser.</p>
              </div>
            </div>
        ...
        <div className="space-y-4 p-4">
          <label className="block text-[10px] font-bold uppercase tracking-wider text-[#888899]">
            API Key
            <input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="AIza..." className="mt-1 w-full rounded-lg border border-[#2A2A38] bg-[#09090D] px-3 py-2 text-xs normal-case text-white outline-none placeholder:text-[#555566] focus:border-teal-400" />
          </label>
          ...
          <div className="rounded-xl border border-[#2A2A38] bg-[#0B0B10] p-3 text-[10px] leading-relaxed text-[#8F8FA0]">
            ...
            <ol className="list-decimal space-y-1.5 pl-4">
              <li>
                Open{" "}
                <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-teal-300 hover:underline">
                  Google AI Studio API keys <ExternalLink size={10} />
                </a>
                .
              </li>
          ...
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-3 text-[10px] leading-relaxed text-amber-100/80">Browser storage is convenient, not equivalent to a backend secret vault. The key is sent only to Clypra Studio AI endpoints and then to Gemini for each request.</div>
        </div>
        ...
        <div className="flex justify-between gap-2 border-t border-[#2A2A38] bg-[#15151C] p-4">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={handleSave} disabled={!apiKey.trim()} className="rounded-lg bg-teal-600 px-3 py-2 text-xs font-bold text-white hover:bg-teal-500 disabled:cursor-not-allowed disabled:opacity-50">
              Save Key
            </button>
          </div>
        </div>
