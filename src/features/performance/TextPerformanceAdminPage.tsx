...
import { ArrowLeft, Check, Clipboard, Database, Download, RefreshCw, Sliders, Type, Volume2, Video } from "lucide-react";
...
export function TextPerformanceAdminPage() {
  ...
  const interactionCount = cohorts.reduce((sum, row) => sum + (row.interactionSampleCount ?? 0), 0);
    ...
    <div className="h-[100dvh] max-h-[100dvh] min-h-0 w-full overflow-x-hidden overflow-y-auto overscroll-contain" style={{ background: "var(--studio-bg)", color: "var(--studio-text)" }}>
      ...
      <main className="mx-auto max-w-7xl space-y-5 px-6 py-8">
        ...
        {!data ? <EmptyState text="The performance API is unavailable. No local or mock values are shown." /> : cohorts.length === 0 ? <EmptyState text="No real text telemetry matches these filters yet. Render text, an effect, or a template in Clypra, then refresh." /> : <>
          <section className="grid gap-3 sm:grid-cols-4"><Metric icon={<Type size={15} />} label="Text renders" value={renderCount.toLocaleString()} detail={`${data.totalWindows.toLocaleString()} API windows`} /><Metric icon={<Sliders size={15} />} label="Interactions" value={interactionCount.toLocaleString()} detail="completed editor transactions" /><Metric icon={<Database size={15} />} label="API observations" value={data.totalApiSamples.toLocaleString()} detail="deduplicated live windows" /><Metric icon={<RefreshCw size={15} />} label="Last received" value={latest ? new Date(latest).toLocaleTimeString() : "—"} detail="live API timestamp" /></section>
          <RelatedPerformance audio={related.audio} preview={related.preview} />
          <section className="overflow-hidden rounded-2xl border border-(--studio-border) bg-(--studio-panel)"><div className="border-b border-(--studio-border) px-5 py-4"><h2 className="text-sm font-bold text-white">Text rendering and interaction comparison</h2><p className="text-xs text-(--studio-muted)">{data.totalWindows.toLocaleString()} real windows · render and editor transaction cohorts are kept separate · frame SLA P95 ≤16.67 ms · interaction SLA P95 ≤100 ms</p></div><div className="min-w-0 overflow-x-auto"><table className="min-w-[2200px] w-full text-left text-xs"><thead className="border-b border-(--studio-border) text-[10px] font-bold uppercase tracking-wider text-(--studio-muted)"><tr>{["Text kind / path", "Environment / phase", "Operation / property", "Windows / renders", "Render P50 / P95 / P99", "Interaction P95 / input→preview", "Stage P95 (ms)", "Cache hit", "Bottleneck / coverage", "Confidence / SLA"].map((heading) => <th key={heading} className="px-5 py-3">{heading}</th>)}</tr></thead><tbody className="divide-y divide-(--studio-border)">{cohorts.map((row) => <tr key={`${row.kind}-${row.rendererPath}-${row.runtimeEnvironment}-${row.phase}-${row.operation}-${row.property || "none"}`}><td className="px-5 py-4"><strong className="text-white">{kindLabel[row.kind]}</strong><div className="text-(--studio-muted)">{row.rendererPath}</div></td><td className="px-5 py-4 text-(--studio-muted)">{row.runtimeEnvironment}<br />{row.phase}</td><td className="px-5 py-4 text-(--studio-muted)">{row.operation}<br />{row.property || "—"}</td><td className="px-5 py-4 text-white">{row.sampleCount} windows<br /><span className="text-(--studio-muted)">{row.renderCount.toLocaleString()} renders · {row.interactionSampleCount ?? 0} interactions</span></td><td className="px-5 py-4 font-semibold text-white">{row.renderSampleCount > 0 ? `${ms(row.p50RenderTimeUs)} / ${ms(row.p95RenderTimeUs)} / ${ms(row.p99RenderTimeUs)}` : "—"}</td><td className="px-5 py-4 font-semibold text-white">{row.interactionSampleCount > 0 ? <>{ms(row.interactionP95Us)}<br /><span className="text-(--studio-muted)">{ms(row.inputToPreviewP95Us)}</span></> : "—"}</td><td className="px-5 py-4 text-(--studio-muted)">Font {ms(row.p95FontWaitUs)} · Compile {ms(row.p95CompileUs)}<br />Raster {ms(row.p95RasterUs)} · Readback {ms(row.p95ReadbackUs)}<br />Transfer {ms(row.p95TransferUs)} · Paint {ms(row.p95PaintUs)}</td><td className="px-5 py-4 text-white">{row.renderSampleCount > 0 ? `${(row.cacheHitRatio * 100).toFixed(1)}%` : "—"}</td><td className="px-5 py-4 font-semibold text-amber-200">{row.renderSampleCount > 0 ? row.bottleneck : row.interactionBottleneck}<br /><span className="text-(--studio-muted)">{row.stageCoverage}</span></td><td className={row.meetsSLA ? "px-5 py-4 font-semibold text-emerald-300" : "px-5 py-4 font-semibold text-rose-300"}><div>{row.interactionSampleCount > 0 ? `${row.interactionConfidence} interaction` : row.confidence}</div><div>{row.meetsSLA ? "Passing" : "Investigate"}</div></td></tr>)}</tbody></table></div></section>
        </>}
      </main>
    </div>
  );
