import { TextEffectConfig, textEffectConfigToScene } from "@clypra-studio/engine";
...
export function getEffectRepresentation(cfg: TextEffectConfig) {
  ...
  if (cfg.shadowEnabled) {
    tags.push(cfg.shadowType === "drop" ? "dropshadow" : "innershadow");
  }
  if (customRenderer) {
    tags.push(customRenderer.toLowerCase().replace("engine", ""));
  }
  if (cfg.fontFamily) {
    tags.push(cfg.fontFamily.toLowerCase().replace(/\s+/g, "-"));
  }
  while (tags.length < 4) {
    tags.push("clypra-effect");
  }
 ...
 * Auto-generated by Clypra Text Effect Studio on ${new Date().toISOString().split("T")[0]}
 * File: ${className}.ts
 * SceneDocument snapshot (layer graph source of truth for studio re-import):
 * ${sceneSnapshot.slice(0, 200)}...
 */
...
export class ${engineName} {
  ...
  drawFrame(
    ...
    const pulse = 0.88 + 0.12 * Math.sin(this.time * Math.PI * 2);
    ctx.save();
